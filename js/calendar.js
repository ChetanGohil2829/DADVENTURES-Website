...
(function(){
  ...
  function render(root, data){
    const head=el('div','cal-head'); const title=el('h3'); title.textContent='Events Calendar';
    ...
    function add(ev){
      ...
      card.innerHTML=`<h4>${ev.title}</h4><div><span class="badge">${ev.date}</span><span class="badge">${ev.time}</span><span class="badge">${ev.route}</span></div><p>${ev.description||''}</p><a class="badge" href="events.html">Details</a>`;
